...
AItem* AItem::GetClosestInteractableItem(ACharacter* fromCharacter)
{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("FromCharacter가 null"));
		return nullptr;
	}
...
void AItem::UseItem()
{
	...
	if (player)
	{
		...
		if (playerEquidItem)
			...
			GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("PlayerEquidItem component가 존재하지 않음"));
	}
