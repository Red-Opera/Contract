...
void AHealPack::AddHealPack()
{
	...
	GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Green, TEXT("Heal Pack이 인벤토리에 추가됨"));
...
void AHealPack::RemoveHealPack()
{
	if (itemMesh == nullptr)
	{
		GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, TEXT("Item mesh가 존재하지 않음"));
		return;
	}
...
void AHealPack::UseItem()
{
    if (!player)
    {
		UE_LOG(LogTemp, Warning, TEXT("Player character가 존재하지 않음"));
        return;
    }
    ...
    (
        -1, 5.f, FColor::Green,
		FString::Printf(TEXT("체력 회복: %d -> %d. 최대 체력 : %d"), currentHealth, newHealth, maxHealth)
    );
