...
UEnemyAnimation::UEnemyAnimation()
{
    // 초기값 설정
    Speed = 0.0f;
    Direction = 0.0f;
...
void UEnemyAnimation::NativeInitializeAnimation()
{
    ...
    // 소유 캐릭터 확인 로직 추가
    if (!owningCharacter)
        GEngine->AddOnScreenDebugMessage(-1, 5.0f, FColor::Red, TEXT("적 애니메이션의 소유 캐릭터가 존재하지 않습니다!"));
    ...
    else
    {
        ...
        (
            -1, 5.0f, FColor::Green, 
            FString::Printf(TEXT("적 애니메이션 소유 캐릭터: %s"), *owningCharacter->GetName())
        );
        ...
        GetWorld()->GetTimerManager().SetTimer(TimerHandle, [this]()
        {
            if (!owningCharacter)
            {
                UE_LOG(LogTemp, Error, TEXT("UEnemyAnimation: 타이머 후에도 캐릭터가 null입니다."));
            }
        }, 0.5f, false);
    }
}
...
void UEnemyAnimation::PlayFireMontage()
{
...
void UEnemyAnimation::PlayReloadMontage()
{
...
void UEnemyAnimation::PlayEquipMontage()
{
...
void UEnemyAnimation::PlayThrowGrenadeMontage()
{
...
void UEnemyAnimation::PlayHitReactMontage(FName HitDirection)
{
...
void UEnemyAnimation::PlayDeathMontage()
{
...
FTransform UEnemyAnimation::GetRightHandIKTransform() const
{
    // 소유 캐릭터가 유효한지 확인
    if (!owningCharacter)
    {
        GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, TEXT("적 캐릭터가 존재하지 않음"));
    ...
    if (!enemy)
    {
        GEngine->AddOnScreenDebugMessage(-1, 1.0f, FColor::Red, TEXT("적 애니메이션: Enemy 캐스트 실패"));
        return FTransform::Identity;
    }

    // Enemy에서 오른손 IK 트랜스폼 가져오기
    return enemy->GetRightHandIKTransform();
}

void UEnemyAnimation::UpdateLeftHandIKCache()
{
    // 초기화
    ...
    if (IsValid(enemy->equippedGun))
    {
        AGun* gun = enemy->equippedGun;
...
// 스레드 안전한 함수로 완전히 수정
FTransform UEnemyAnimation::GetLeftHandIKTransform() const
{
    // 캐시된 값만 반환 (스레드 안전)
